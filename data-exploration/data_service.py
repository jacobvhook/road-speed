...
class CrashDataService:
    ...
    def __init__(self, nyc_app_token: str, from_year: int=2010):
    ...
    def __load_crashes_dataset(self) -> gpd.GeoDataFrame:
        self.crashes = self.data_loader.load_data(data_sources.CRASHES_ENDPOINT, limit=3000000)
        self.crashes = self.crashes[["collision_id", "crash_date", "crash_time", "latitude", "longitude"]]
        self.crashes.dropna(subset=["longitude", "latitude"], inplace=True)
        self.crashes["datetime"] = self.crashes[["crash_date", "crash_time"]].apply(lambda o: self.__get_datetime(o["crash_date"], o["crash_time"]), axis=1)
        self.crashes.drop(columns=["crash_date", "crash_time"], inplace=True)
        self.crashes = self.crashes[self.crashes.datetime >= datetime(self.from_year, 1, 1)]
        self.crashes = gpd.GeoDataFrame(
            self.crashes,
            geometry=gpd.points_from_xy(self.crashes["longitude"], self.crashes["latitude"]),
            crs=geo.STD_EPSG,
        ).to_crs(geo.NYC_EPSG)
    ...
    def __load_streets_dataset(
        ...
        streets = self.data_loader.load_geo_dataframe(
            data_sources.CENTERLINE_ENDPOINT, geometry_column="the_geom", limit=3000000, to_crs=geo.NYC_EPSG
        )
    ...
    def __load_speed_humps_dataset(self) -> gpd.GeoDataFrame:
        humps = self.data_loader.load_geo_dataframe(data_sources.SPEEDHUMPS_ENDPOINT, "the_geom", to_crs=geo.NYC_EPSG, limit=3000000)
    ...
    def __load_speed_limits_dataset(self) -> gpd.GeoDataFrame:
        speed_limits = self.data_loader.load_geo_dataframe(data_sources.SPEEDLIMITS_ENDPOINT, "the_geom", to_crs=geo.NYC_EPSG, limit=3000000)
        speed_limits = speed_limits[["postvz_sl", "geometry"]]
        speed_limits["postvz_sl"] = speed_limits["postvz_sl"].astype(int)
        speed_limits = speed_limits.groupby(by="geometry", as_index=False)["postvz_sl"].mean()
    ...
    def __spatial_join_to_street_data(
        ...
        street_data = street_data.groupby(by=street_data_cols, as_index=False, dropna=False).agg(
            **{agg_column: pd.NamedAgg(column=agg_column, aggfunc=agg_op) }
        )
    ...
    def get_non_intersection_crashes(
        ...
        street_data = self.__load_streets_dataset(columns_to_load=["geometry", "physicalid", "shape_leng"])
        street_data["geometry"] = street_data.geometry.buffer(buffer)
        if join_speed_humps:
            humps = self.__load_speed_humps_dataset()
            street_data = self.__spatial_join_to_street_data(street_data, humps, "humps", "sum")
        ...
        non_intersection_crashes = crashes_street.merge(non_intersection_crashes, on="collision_id", how="inner")
